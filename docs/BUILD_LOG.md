...
| D57 | **The release artifact is one self-contained HTML file**, not a zip of `web/` + `dist/`. A zip must be extracted before it is opened, and Windows lets you double-click `index.html` from inside the compressed-folder preview, where `css/`, `js/` and `../dist/` do not resolve — the page then loads unstyled with no engine, which reads as "the tool is broken" rather than "you skipped a step". One file has no relative paths left to break. | §2.7, §13.1 |
| D58 | **The git tag is the sole source of the version number.** `package.json`'s `version` is deliberately not used: it would have to be bumped in a commit *before* the tag, the step most often forgotten, and the artifact would then be stamped with the previous release's number while claiming to be the new one. A tag cannot disagree with itself. | §7.5 |
| D59 | Packaged releases carry a **build stamp** — version, commit, date, engine contract version and all six schedule vintages — visible in the sidebar and as a comment on the raw file. Output gets attached to disputes (`docs/M25-browser-interface.md` §1); a determination that cannot be tied to the build that produced it is not defensible. The working copy renders nothing there on purpose: unstamped *is* the uncommitted tree. | §7.5, §13.1 |
| D60 | `tools/package-release.mjs` proves its own output rather than trusting the transform: each inlined region is re-derived from the original bytes and must match byte-for-byte, the `<script>`/`<style>` elements must be balanced, and the engine block is **extracted back out of the finished HTML and made to adjudicate a real claim** (`G0463`→J2). Inlining 1.2 MB of script into HTML is exactly where silent corruption would happen, and the escape that makes it safe (`<\/script`, `<\!--`) rests on an argument about JS identity escapes — so the argument is not what the build relies on. | §2.7, §18.2 |
| D61 | `release.yml` **calls `verify.yml`** via `workflow_call` rather than repeating its steps. A release must not be cuttable from a tree that would fail `verify`, and a second copy of the gate is a gate that drifts. | §18.2 |
| D62 | The external-reference check in the packager scans **markup only**, with `<script>` and `<style>` contents stripped. Its first run failed on `app.css`'s own header comment explaining why there is no Google Fonts `<link>` (D51) — a whole-document scan reads that sentence as a surviving external stylesheet. | §2.7 |
| D63 | **`npm run verify` had never run.** It chained `lint:registry` at `tools/lint-registry.mjs`, which does not exist (U18 todo), so the command the README called *the* build gate died with `MODULE_NOT_FOUND` on every invocation. CI passed only because the workflow invokes typecheck and test as separate steps. `verify` is now `typecheck && test`; put the lint back the day U18 lands. `diff:registry` (U20) and `gen:goldens` (U22) are dead scripts for the same reason and are now documented as placeholders. | §18.2 |
| D64 | **Spec §4.3.1's argument table was still rev 11's bare-payload form**, three revisions after rev 13 reversed it: 15 operators documented as taking bare `string`/`number`/`string[]` where the code requires named objects, `optionIs`/`optionAtLeast` naming a `value` field the code calls `equals`/`atLeast`, `flag` marking a required `message` optional, and `hasModifier`/`claimContainsCode` listed under `string[]` when both take a single string. The table is now **derived from `operators.ts`** rather than hand-transcribed. Not cosmetic: authoring from the old table produces a rule that passes load-time validation, faults at evaluation, and reports as an unrelated failure — the exact incident rev 13 existed to prevent, still reachable from the document that caused it. | §4.3.1 |
| D65 | **Three operators the spec offered do not exist.** `chargeAtLeast`, `claimMoneyAtLeast` and `claimDayCountAtLeast` are enumerated in §4.3 but deliberately omitted from `operators.ts` as money-bearing (milestone 1 reports no amounts). Unmarked, a rule authored against one fails to load with no hint the gap was intentional — the failure shape D40 and §9.5 exist to forbid. Now marked deferred in the spec, along with `setAmount`, `multiply`, `setCoinsurance`, `carveOut`, `exclusion`, `lesserOfCandidates`. Neither review pass caught this; it surfaced while fixing D64. | §4.3, §9.5 |
| D66 | **`OPPS.PKG.Q4.COMPANION` crashed the engine to zero output** on any claim carrying a high-rate Q1/Q2 line, a lower-rate T/S/V line, and a Q4 line (`0101T G0516 0002M`, `68841 0640T 0002M`). Its `bundleUnder.among` was a bare `siIn` with no `not(statusIn ['BUNDLED'])` guard — unlike its sibling `OPPS.PKG.Q.SURVIVOR_TIEBREAK` — so it ranked over a line band 4000a had already bundled and faulted. **Adding the guard alone was insufficient:** `bundleUnder`'s `among` ranks over the rule's *declared epoch's frozen snapshot*, so at subBand a / E2 the guard could never see subBand a's own bundling. The rule moved to subBand b / E3a (`order` 4350) — the same relocation SURVIVOR_TIEBREAK already uses for this exact reason. The other four `bundleUnder` rules were checked and are safe by construction (mutually exclusive `when` clauses over the frozen SI census). | §9.2 |
| D67 | **§12.8's containment guarantee was defeated by the auditability layer.** The D66 fault *was* caught line-locally as §12.8 requires — but the halted line's `Evaluation` for an unrelated rule carried a line-specific counterfactual ("halted by an earlier engine fault...") while other lines carried that rule's normal `when`-derived string. `collectCounterfactuals` dedupes by `ruleId` and throws on two different strings — a correct, load-bearing invariant (§5.3a's `standard`-level compression stores one string per rule) — so the assertion was right and the data feeding it was wrong. A halt reason is not a counterfactual: it is now `null`, with the reason recoverable via `outcome: 'SKIPPED'` plus `examined.detail`. The invariant was left untouched. **The lesson generalises:** an invariant that hard-throws inside trace assembly converts any contained line fault into total claim loss. | §12.8, §5.3a |
| D68 | **Garbage codes were silently rewritten into clean payable lines.** `normalizeProcCode()` peeled any trailing `[A-Z0-9]{2}` off a 7-char token as a modifier with no validation, so `9928401` adjudicated as a clean `PAID` `99284` with zero flags — and `malformedReason()`'s `length > 5` concatenation branch was **unreachable at length 7**, the length where concatenation actually happens. Now: a shape check gates the peel, the concatenation branch is reachable, and every peel raises `OPPS.CLASSIFY.MODIFIER_PEELED` (`assumption`) naming the raw token, the adjudicated code and the peeled modifier. **The protection is the disclosure, not the shape check** — with no CMS modifier table in the repo the gate can only reject numeric pairs below 20, an *unsourced* claim that no CPT modifier is `00`-`19`. `9928499` still peels. Get a modifier list into `gen-data.mjs` and make this a real lookup; until then treat the `< 20` rule as unverified policy. | §8.1 |
| D69 | **A Q4 conversion claimed a fee schedule the data cannot back.** `basis` was forced to `CLFS` for every unpackaged-Q4 conversion, and §9.3 *documented that as deliberate* — circularly, since the assertion existed to stop the resolver contradicting the section. Two groups were conflated under "24 Q4 codes with no usable rate": **18** are CLFS-present at `RATE=0.00`/`INDICATOR=L`, where `CLFS` is truthful and the resolver's `ROUTED_UNKNOWN` genuinely is a limitation to route around; **6** (`0602T`, `0603T`, `81099`, `84999`, `85999`, `88749`) match no row in CLFS *or any other loaded schedule*, where the resolver was never wrong and `CLFS` fabricated provenance. Rev 12's fix was right for the 18 and overcorrected onto the 6. Now `lookupClfs` presence decides, and the 6 report `ROUTED_UNKNOWN` with `OPPS.Q4.NO_SCHEDULE_MATCH` (`gap`). Never-`OPPS_APC` unaffected. **This reverses documented behaviour** — ratified as a correction, not a regression. | §9.3 |
| D70 | **`engineStatus: 'PARTIAL'` (§18.29a) had no test.** Hard-coding it to `'OK'` left all 387 tests green — verified by mutation. A missing feature announces itself; an untested guarantee reads as covered. Now asserted by D66's containment regression test, which was confirmed to fail when D67's fix is reverted. Also pruned 3 orphaned `cli.test.ts` snapshots left by an old test rename, which `npm test` had been reporting as obsolete for the whole build. | §18.29a, §15.2 |
| D71 | **Still open from the deep review, not fixed here:** `E*:liveSet` ignores halt state (`evaluate.ts` checks only `status`, never `haltReason`), contradicting its own documented invariant — latent, since no shipped rule uses `stop`, but reachable through the public `evaluate()` API. And `NOT_REACHED` remains declared in `types.ts` and emitted nowhere, so a phase-1-rejected line gets zero trace rows and §18.18 ("reserved rules appear in *every* trace") is unmet. | §2.5, §18.18 |
